...
import re
import json
from DateCrawlerHelpers import (
    getHTMLdocument,
    GetText,
    ContainsSubstring,
    GetWeeks,
...
for ObjectTag in ObjectTags:
    ...
    tdTag = ObjectTag.find(
        "td", attrs={"headers": re.compile(rf'\b{"activity"}\b', re.I)}
    )
    ...
    if (not ActivityType) or (
        not ContainsSubstring(ActivityType.lower(), classifications)
    ):
        continue
...
AllDates = [
    ...
    if (Event[0].lower() not in ["semester 1", "semester 2", "non-teaching"])
]
...
for Event in KeyDates:
    if Event[0] == "Semester 1":
        ...
        Sem2Dates = Event[1]
    elif ("Easter" in Event[0] or "Semester 1" in Event[0]) and (
        "non-teaching" in Event[0].lower()
    ):
        MidSem1 = Event[1]
